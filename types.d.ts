...
export interface DeletegateOptions {
  ...
  beforePostbackSend?(postback: any): any;
  beforeSend?<T extends MessageUnion>(message: MessageUnion): T;
  beforeWebviewDisplay?(uri: string, message: MessagePayload, event: React.MouseEvent<any>): void | undefined | string | boolean;
}
...
export interface StackchatInitOptions {
  ...
  // Tracks whether to reopen the messenger on a new page or not, defaults to true.
  trackOpenState?: boolean;
  /** User's id */
  userId?: string;
